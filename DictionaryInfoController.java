...
public class DictionaryInfoController {
  ...
  private void ShowResults(ArrayList<String> arr) {
    ...
    for (var i : arr) {
      int length = i.length();
      if (length == 6) Words_Six_Letters++;
      else if (length > 6 && length < 10) Words_Seven_To_Nine_Letters++;
      else Words_At_Least_10_Letters++;
    }
    String[] pourcentages = {String.format("%.2f",(double)Words_Six_Letters/TotalWords*100), String.format("%.2f",(double) Words_Seven_To_Nine_Letters/TotalWords*100), String.format("%.2f",(double) Words_At_Least_10_Letters/TotalWords*100)};
